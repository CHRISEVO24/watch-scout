...
const OUT_FILE = path.join(DATA_DIR, "inventory-latest.json");
...
// WPB's inventory tracker now runs independently on GitHub (cron jobs,
// no laptop dependency) - history.json is hosted there, not in a local
// iCloud folder anymore. Fetch from the live source instead of a stale
// local path.
const HISTORY_URL = args.url || "https://chrisevo24.github.io/wpb-tracker/history.json";
...
async function run() {
  console.log(`Fetching WPB inventory from: ${HISTORY_URL}`);
  const { data: history } = await axios.get(HISTORY_URL, { timeout: 20000 });
  const timestamps = Object.keys(history).sort();
  if (timestamps.length === 0) {
    console.error("No snapshots found in history.json.");
    process.exit(1);
  }
  const latestTimestamp = timestamps[timestamps.length - 1];
  const snapshot = history[latestTimestamp];
  console.log(`Using most recent snapshot: ${latestTimestamp} (${Object.keys(snapshot).length} products)`);
  ...
  console.log(`Done. ${inventory.length} inventory items loaded (${inStockCount} in stock) — written to data/inventory-latest.json`);
}

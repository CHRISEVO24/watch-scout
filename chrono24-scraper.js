...
async function scrape() {
  ...
  // Load existing data — do NOT pre-load URLs into globalSeen
  // so that re-scraped brands can be freshly collected
  let existing = [];
  ...
  for (const brand of BRANDS) {
    ...
    // Collect fresh listings for this brand FIRST
    ...
    // Only replace existing brand listings if we got results back
    if (freshItems.length > 0) {
      ...
      // Save after each brand so partial runs preserve progress
  ...
  // Update history with any new items
  const allFresh = existing; // existing now contains all merged data
